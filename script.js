...
const AutoSave = {
    ...
    startAutoSave() {
        // Sauvegarder toutes les 30 secondes (uniquement authentification si case cochée)
...
async function sendData(mode) {
    try {
        ...
        if (response.ok) {
            if (mode === 'test') {
                // Mode Test : Afficher le Google Sheet
...
window.addEventListener('beforeinstallprompt', (e) => {
    ...
    if (installButton) {
        ...
        setTimeout(() => { Toast.info('Installez DictaMed pour un accès rapide et hors ligne !', 'Installation disponible', 8000); }, 500);
});
