 ...
 * Version: 1.2.1 - Améliorations de synchronisation, gestion d'erreurs et prévention des memory leaks
...
class AdminNavigationManager {
    constructor() {
        ...
        this.eventListeners = []; // Track event listeners for proper cleanup
        this.initPromise = null; // Promise to prevent race conditions
    }

    /**
     * Initialisation du gestionnaire de navigation admin avec gestion d'erreurs améliorée et prévention des race conditions
     */
    async init() {
    ...
    async _performInit() {
        try {
            console.log('🔧 Initialisation AdminNavigationManager v1.2.1...');
            ...
            console.log('✅ AdminNavigationManager v1.2.1 initialisé avec succès');
            return true;
    ...
    createAdminButton() {
        try {
            ...
            adminBtn.addEventListener('click', clickHandler);
    ...
    /**
     * Liaison de l'écouteur d'état d'authentification amélioré avec tracking des listeners
     */
    ...
    /**
     * Configuration optimisée de l'écouteur Firebase Auth avec gestion des erreurs améliorée
     */
        ...
        const trySetupListener = () => {
            ...
            if (typeof window.FirebaseAuthManager !== 'undefined' && 
                ...
                try {
                    ...
                    window.FirebaseAuthManager.addAuthStateListener(authStateHandler);
                    
                    // Store the handler for cleanup
                    this.eventListeners.push({
                        element: window.FirebaseAuthManager,
                        event: 'authStateListener',
                        handler: authStateHandler
                    });
    ...
    showAdminButton() {
        if (this.adminNavBtn) {
            ...
            try {
                ...
                console.warn('⚠️ Animation non supportée:', animationError);
            }
    ...
    /**
     * Nettoyage complet des ressources avec prévention des memory leaks
     */
    cleanup() {
        try {
            ...
            this.eventListeners.forEach(listener => {
                try {
                    ...
                    console.warn('⚠️ Erreur lors de la suppression d\'un event listener:', error);
                }
            });
            this.eventListeners = [];
